...
def detect_and_analyze_posture(frame):
    ...
    for result in results:
        ...
        for i, person in enumerate(result.keypoints):
            ...
            if is_sitting: 
                print( " ------ this person is sitting. " ) 
                print( str( posture_slope_or_angle )) 
...
def analyze_sitting_posture(keypoints):
    ...
    try:
        ...
        # Check angle of body parts to determine sitting posture            
        ...
        if knee_hip_shoulder_angle_left < 100 or knee_hip_shoulder_angle_right < 100:
            print( "  ------Sitting & Angle: " + str(knee_hip_shoulder_angle_left) ) 
            return True 
            
        # Check alignment of body parts to determine sitting posture
        # the [0, 0] coordinate locates on the top left corner, so 
        if left_knee[1] < left_hip[1] or right_knee[1] < right_hip[1]:
            print("   ------Sitting & LKnee: " + str( left_knee[1]) + " LHip: " + str( left_hip[1]) )
            return True
    ...
    try:
        ...
        if is_sitting and left_shoulder.all() > 0 and left_ear.all() > 0:
            ...
            if head_to_neck_slope_left < 0.4 and abs(left_shoulder[1] - right_shoulder[1]) < 20:
                ...
                return False, head_to_neck_slope_left
            ...
            if head_to_neck_slope_right < 0.4 and abs(left_shoulder[1] - right_shoulder[1]) < 20:
                ...
                return False, head_to_neck_slope_right
        else:
            ...
            if head_to_shoulder_to_hip_angle > 150:      # and abs(left_shoulder[1] - right_shoulder[1]) < 20:
                ...
                return False, head_to_shoulder_to_hip_angle
...
def draw_sitting_posture_lines(frame, keypoints):
    ...
    try:
        ...
        left_ear       = keypoints[0][3]    # left eye (x, y)
        right_ear      = keypoints[0][4]    # right eye (x, y)
        left_shoulder  = keypoints[0][5]    # Left shoulder (x, y)
        right_shoulder = keypoints[0][6]    # Right shoulder (x, y)

        # Ensure keypoints are valid before drawing lines
        if all(kp[0] > 0 and kp[1] > 0 for kp in [left_ear, left_shoulder]):
        ...
        if all(kp[0] > 0 and kp[1] > 0 for kp in [ right_ear, right_shoulder]):
            # Draw lines: Head to Neck and Neck to Shoulders
...
############################################################## 
def analysis_one_video(input_video_path): 
    # Specify the path to the input video file
    print(input_video_path)  #= "/home/jeff/***.avi"

    # Specify the path to save the output video    
    #output_video_path = "/home/jeff/***.avi"  # Replace with your desired output path
    output_video_path = os.path.splitext(input_video_path)[0] +"_POSTURE.avi" 
...
def main():
       ...
       videos_directory = "/home/jeff/HomeSecurity_VideoRecording/videos/20240909"
